...
parser.add_argument(          # 由0.01改为0.001——xiaowen
...
resource.setrlimit(resource.RLIMIT_NOFILE, (4096, rlimit[1]))  #the original value is 30000
...
def model_fn_decorator(
    ...
    def model_fn(
        ...
        with torch.set_grad_enabled(not is_eval):
            ...
            acc_rgbd = (cls_rgbd == labels).float().sum() / labels.numel()  # 分类精度
...
class Trainer(object):
    ...
    def eval_epoch(self, d_loader, is_test=False, test_pose=False, it=0):       # 模型评估
        self.model.eval()
        ...
        if is_test:
            ...
            with open(os.path.join(config.log_eval_dir, seg_res_fn), 'w') as of:
                for k, v in acc_dict.items():
                    print(k, v, file=of)
        # if args.local_rank == 0:        会报错 所以注释了
        ...
        with tqdm.tqdm(range(config.n_total_epoch), desc="%s_epochs" % args.cls) as tbar, tqdm.tqdm(
            ...
            for epoch in tbar:
                ...
                for batch in train_loader:
                    ...
                    # if args.gradient_clip > 0:    #  梯度裁剪  (adde by xiaoWen)
                    #     torch.nn.utils.clip_grad_norm_(self.model.parameters(), args.gradient_clip)
                    #     print('gradient clip')
                    self.optimizer.step()
