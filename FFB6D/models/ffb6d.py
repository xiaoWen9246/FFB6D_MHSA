...
class FFB6D(nn.Module):
    def __init__(
        ...
        # ######################## prepare stages#########################
        self.n_cls = n_classes      # 分类的类别数  
        self.n_pts = n_pts         # 回归的点数
        self.n_kps = n_kps     #  关键点的数量
        cnn = psp_models['resnet50'.lower()]()        # 由resnet34修改为resnet50——小温

        rndla = RandLANet(rndla_cfg)  #  点云聚类和分割的特征提取器

        self.cnn_pre_stages = nn.Sequential(      # 取Resnet的conv1, bn1, relu和maxpool层对数据进行预处理
            cnn.feats.conv1,  # stride = 2, [bs, c, 240, 320]
            cnn.feats.bn1, cnn.feats.relu,
            cnn.feats.maxpool  # stride = 2, [bs, 64, 120, 160]
        )
        self.rndla_pre_stages = rndla.fc0   #  取RandLANet的fc0层作为数据预处理

        # ####################### downsample stages#######################
        self.cnn_ds_stages = nn.ModuleList([
            cnn.feats.layer1,    # stride = 1, [bs, 64, 120, 160]    bs——batch_size
        ...
        self.ds_sr = [4, 8, 8, 8]    #  表示不同阶段的下采样率

        self.rndla_ds_stages = rndla.dilated_res_blocks  
        self.ds_rgb_oc = [256, 512, 2048, 1024]       #修改为Resnet50每个阶段RGB的通道数
        #self.ds_rgb_oc = [64, 128, 512, 1024]  # 每个阶段RGB的通道数  
        self.ds_rndla_oc = [item * 2 for item in rndla_cfg.d_out]  # 每个阶段RandLA的通道数   [64, 128, 256, 512]
        #  定义4个ModuleList, 用于RGB和RandLA特征的融合   每个nn.ModuleList包含2个卷积层
        self.ds_fuse_r2p_pre_layers = nn.ModuleList()      #  用于将RandLA特征融合到RGB中
        self.ds_fuse_r2p_fuse_layers = nn.ModuleList()     #
        self.ds_fuse_p2r_pre_layers = nn.ModuleList()      #   用于将RGB特征融合到RandLA中
