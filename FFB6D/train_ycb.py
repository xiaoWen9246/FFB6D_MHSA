...
parser.add_argument('-pretrained', type=str, default=False, help="是否加载作者提供的预训练模型")
...
def load_checkpoint(model=None, optimizer=None, filename="checkpoint"):
    ...
    if os.path.isfile(filename):
        ...
        checkpoint = torch.load(filename,map_location='cuda:{}'.format(args.local_rank))
...
def load_pretrined(model=None):     # added by xiaowen   由作者提供的预训练模型初始化我的模型参数
    net_dict = model.state_dict()
    predict_model = torch.load('/home/wenyh/Desktop/xiaowen/ffb6d/models/FFB6D_best_pretrained/FFB6D_best.pth.tar')
    state_dict = {k : v for k,v in predict_model.items() if k in net_dict.keys()}
    net_dict.update(state_dict)
    model.load_state_dict(net_dict)
    print("loading FFB6D_best pretrained mdl.")
...
def model_fn_decorator(
    ...
    def model_fn(
        ...
        with torch.set_grad_enabled(not is_eval):
            ...
            acc_rgbd = (cls_rgbd == labels).float().sum() / labels.numel()    # calculates the accuracy (acc_rgbd) of a predicted RGBD segmentation (cls_rgbd) compared to the ground truth labels (labels)
    ...
    def eval_epoch(self, d_loader, is_test=False, test_pose=False, it=0):
        ...
        if is_test:
            ...
            for k, v in acc_dict.items():
                seg_res_fn += '_%s%.2f' % (k, v[0]) # v修改为了v[0]
...
def train():
    ...
    optimizer = optim.Adam(
        model.parameters(), lr=args.lr, weight_decay=args.weight_decay, eps=1e-3
    )
    ...
    if not args.eval_net:
        ...
        lr_scheduler = CyclicLR( 
            optimizer, base_lr=1e-5, max_lr=1e-3,         #  默认是1e-5~1e-3
            ...
            #step_size_up=train_ds.minibatch_per_epoch // args.gpus * 4,
            #step_size_down=train_ds.minibatch_per_epoch // args.gpus * 4,
            #mode='triangular'
            mode='triangular'
        )
    else:
